...
const JWT_SECRET = process.env.JWT_SECRET || 'yar-emtehan-default-secret-please-change-2026';

if (!GEMINI_API_KEY) {
  console.warn('⚠️ متغیر GEMINI_API_KEY تنظیم نشده. سایت بالا میاد ولی چت کار نمی‌کنه تا این کلید رو توی Render اضافه کنی.');
}
if (!process.env.JWT_SECRET) {
  console.warn('⚠️ متغیر JWT_SECRET تنظیم نشده؛ از یه مقدار پیش‌فرض استفاده می‌شه. برای امنیت بیشتر، توصیه می‌شه توی تنظیمات Render خودت یکی بذاری، ولی سایت الان کار می‌کنه.');
}
...
// تلاش خودکار مجدد وقتی Gemini موقتاً شلوغه (خطای 429 یا 503)
async function callGeminiWithRetry(body, maxRetries = 3) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    // صبر تصاعدی: ۱ ثانیه، ۲ ثانیه، ۴ ثانیه...
    const waitMs = 1000 * Math.pow(2, attempt);
    console.warn(`Gemini شلوغه (تلاش ${attempt + 1}/${maxRetries + 1})، ${waitMs}ms صبر می‌کنیم...`);
...
app.post('/api/chat', authMiddleware, upload.single('file'), async (req, res) => {
  try {
    if (!GEMINI_API_KEY) {
      return res.status(500).json({ error: 'کلید GEMINI_API_KEY روی سرور تنظیم نشده. توی تنظیمات Render اضافه‌اش کن.' });
    }
    ...
    if (!response.ok) {
      ...
      const friendlyMsg = /high demand|quota|overloaded/i.test(rawMsg)
        ? 'سرورهای هوش مصنوعی فعلاً خیلی شلوغن. چند لحظه صبر کن و دوباره بفرست 🙏'
        : rawMsg || 'خطای نامشخص از سرویس هوش‌مصنوعی';
